...
public class CandidateService {
    ...
    public List<CandidateOutput> getAll() {
        ...
        List<CandidateOutput> candidates =  candidateList.stream().map(this::toCandidateOutput).collect(Collectors.toList());
        for(int i = 0; i < candidates.size(); i++){
            try {
                ...
                candidates.get(i).setPartyOutput(partyOutput);
                candidates.get(i).setElectionOutput(electionOutput);
